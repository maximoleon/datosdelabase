...
class ControladorProducto extends ControladorGeneral {
    ...
    public function cantidad($datos = null) {
        try {
            $resultado = $this->refControladorPersistencia->ejecutarSentencia(DbSentencias::CANTIDAD_PRODUCTOS);
            $arrayProductos = $resultado->fetchAll(PDO::FETCH_ASSOC);
            $return = array();
            foreach ($arrayProductos as $row) {
                $return[] = array('cantida' => $row['COUNT(*)']);
            }
            return $arrayProductos;
        } catch (Exception $e) {
            throw new Exception("Producto-contar: " . $e->getMessage());
        }
    }
